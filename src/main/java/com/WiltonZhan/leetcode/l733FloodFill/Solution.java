...
public class Solution {
    public int[][] floodFill(int[][] image, int sr, int sc, int newColor) {
        ...
        floodFill(image, sr, sc, image[sr][sc], newColor, new boolean[image.length][image[0].length]);
        return image;
    }

    private void floodFill(int[][] image, int sr, int sc, int oldColor, int newColor, boolean[][] visited) {
        if (0 <= sr && sr < image.length && 0 <= sc && sc < image[0].length && image[sr][sc] == oldColor && !visited[sr][sc]) {
