...
public class Solution {
    /**
     * 构建有向图，然后检测途中是否存在环。若存在返回false,否则返回true
     */
    public boolean canFinish(int numCourses, int[][] prerequisites) {
        ...
        for (int i = 0; i < numCourses; i++) {
            if (hasCycle(adjacencyList, visited, i)) {
                return false;
            }
        }
    ...
    private boolean hasCycle(LinkedList<Integer>[] adjacencyList, int[] visited, int v) {
        if (visited[v] == 1) {  // 正在搜索
            return true;
        } else if (visited[v] == 2) {   // 搜索完成
            return false;
        } else {    // 未搜索
            visited[v] = 1;
